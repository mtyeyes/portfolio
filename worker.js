...
const prepareScriptForDemonstration = (script) => {
  const minifyScript = (str) => {
    ...
    return str
      .replace(regexForComments, '')
      .replace(regexForNewlinesAndWhitespaces, '');
  };
  ...
  const highlightMethods = (str) => {
    ...
    const encaseMethodsInSpan = (match, offset, string) => {
      if (isQuerySelectorParameter(offset, string)) {
        return match;
      } //this would be replaced with (?<!\') as soon as most browsers would implement lookbehind
